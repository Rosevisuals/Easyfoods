...
struct PrimaryButton: View {
    ...
    init(_ title: String, icon: String? = nil, isFullWidth: Bool = true, isSmall: Bool = false, action: @escaping () -> Void) {
        self.title = title; self.icon = icon; self.isFullWidth = isFullWidth
        self.isSmall = isSmall; self.action = action
    }

    var body: some View {
        Button(action: action) {
            HStack(spacing: 6) {
                if let icon { Text(icon).font(.system(size: isSmall ? 12 : 14)) }
                Text(title).font(isSmall ? EFFont.semibold(12) : EFFont.bold(14))
            }
            .foregroundStyle(.white)
            .padding(.horizontal, isSmall ? 14 : 20)
            .padding(.vertical, isSmall ? 9 : 13)
...
struct GhostButton: View {
    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                .font(EFFont.semibold(13)).foregroundStyle(Color.inkSecond)
                .padding(.horizontal, 20).padding(.vertical, 12)
                .frame(maxWidth: .infinity)
                .background(Color.bgCard)
                .clipShape(RoundedRectangle(cornerRadius: EFRadius.md))
                .overlay(RoundedRectangle(cornerRadius: EFRadius.md)
                    .stroke(Color.inkQuartern.opacity(0.4), lineWidth: 1.5))
        }
        .buttonStyle(SpringButtonStyle())
    }
}
        ...
        HStack(spacing: 12) {
            Button {
                ...
                Image(systemName: "minus")
                    .font(.system(size: 12, weight: .bold)).foregroundStyle(Color.inkPrimary)
                    .frame(width: 30, height: 30).background(Color.bgSubtle)
                    .clipShape(RoundedRectangle(cornerRadius: 9))
                    .overlay(RoundedRectangle(cornerRadius: 9).stroke(Color.inkQuartern.opacity(0.3), lineWidth: 1))
            }
            .buttonStyle(SpringButtonStyle())

            Text("\(quantity)")
                .font(EFFont.bold(16)).foregroundStyle(Color.inkPrimary).frame(minWidth: 24)
...
// MARK: - Liquid Glass — use ONLY over images / coloured backgrounds
    ...
    func body(content: Content) -> some View {
        content
            ...
            .overlay(LinearGradient(
                colors: [.white.opacity(0.40), .clear, .white.opacity(0.10)],
...
struct RatingBadge: View {
    ...
    var body: some View {
        HStack(spacing: 3) {
            ...
            Text(String(format: "%.1f", rating)).font(EFFont.bold(11)).foregroundStyle(Color.inkPrimary)
        }
        .padding(.horizontal, 8).padding(.vertical, 4).liquidGlass(cornerRadius: 20)
    }
}
...
struct TagChip: View {
    let label: String
    var body: some View {
        Text(label).font(EFFont.semibold(11)).foregroundStyle(Color.brand)
...
struct SectionHeader: View {
    ...
    var body: some View {
        HStack {
            Text(title).font(EFFont.bold(15)).foregroundStyle(Color.inkPrimary)
            Spacer()
            if let onAction {
                Button("See All", action: onAction).font(EFFont.semibold(12)).foregroundStyle(Color.brand)
            }
        }
...
// MARK: - Tab Bar — WHITE background, SF Symbols, always visible
struct FloatingTabBar: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            .padding(.bottom, 8) // accounts for home indicator
            .background(Color.bgCard) // solid white — always readable
        }
        .background(Color.bgCard)
    }
}
